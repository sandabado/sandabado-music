...
export function InternalBackdrop({ solid = "dodecahedron", color = "109,74,255", lighter = false }: { solid?: BackdropSolid; color?: string; lighter?: boolean }) {
  ...
  return <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden bg-[var(--void)]"><div className={`absolute inset-0 ${lighter ? "opacity-55" : "opacity-35"}`}><PlatonicScene active={solid} height="100%"/></div><div className="absolute inset-0" style={{ background:veil }}/></div>
}
